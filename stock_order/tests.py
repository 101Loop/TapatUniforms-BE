...
class TestStockOrder(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create(
            username="lohitbura", mobile="9729018415", email="lohitbura@gmail.com"
        )
        cls.school = School.objects.create(
            name="MPPS",
            longitude="27.2323232",
            lat="72.324234",
            address="Kurukshetra",
            created_by=cls.user,
        )
        cls.register = User.objects.create(
            username="klaus",
            name="Niklaus",
    ...
    def test_api(self):
        ...
        response = client.post(
            "/api/user/login/",
            data=json.dumps({"username": "klaus", "password": "klaus@123"}),
